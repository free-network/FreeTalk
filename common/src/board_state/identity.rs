//! Portable identity export/import for freetalk boards.
//!
//! This module provides the `IdentityExport` struct that allows users to export
//! their board identity (signing key, membership proof, invite chain) as a portable
//! token that can be imported into another client.
...
const ARMOR_BEGIN: &str = "-----BEGIN FREETALK IDENTITY-----";
const ARMOR_END: &str = "-----END FREETALK IDENTITY-----";
const LINE_WIDTH: usize = 64;

/// A portable identity bundle containing everything needed to restore
/// a user's board identity on a different client.
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct IdentityExport {
    /// The board owner's verifying key (identifies which board)
    pub board_owner: VerifyingKey,
...
impl IdentityExport {
    ...
    /// where the inviter is either the board owner or another chain member.
    fn validate_invite_chain(&self) -> Result<(), String> {
        let owner_id = MemberId::from(&self.board_owner);
        ...
        vk_by_id.insert(owner_id, self.board_owner);
...
mod tests {
    use super::*;
    use crate::board_state::member::{Member, MemberId};
    use crate::board_state::member_info::MemberInfo;
    ...
    fn test_roundtrip_armored() {
        ...
        assert_eq!(decoded.board_owner.as_bytes(), export.board_owner.as_bytes());
        assert_eq!(

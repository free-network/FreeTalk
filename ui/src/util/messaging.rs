...
pub async fn send_message(
    ...
    room_key: RoomKey,
    self_sk: SigningKey,
    room_state_clone: ChatRoomStateV1,
    is_private: bool,
            ...
            if !is_in_members {
                if let Some(ref authorized_member) = room_data.self_authorized_member {
                    let current_member_ids: std::collections::HashSet<_> = room_data
                        ...
                        .map(|m| m.member.id())
                        .collect();
                    let mut members_to_add: Vec<river_core::room_state::member::AuthorizedMember> = vec![authorized_member.clone()];
                    ...
                    let authorized_info: AuthorizedMemberInfo = if let Some(ref stored_info) = room_data.self_member_info {
                        stored_info.clone()
                    } else {
                    ...
                    (
                        Some(river_core::room_state::member::MembersDelta::new(members_to_add)),

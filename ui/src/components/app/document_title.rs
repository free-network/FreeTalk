...
//!
//! Handles:
//! - Setting document.title to board name when a board is selected
//! - Setting document.title to "FreeTalk" when no board is selected
...
/// Global signal tracking total unread messages across all boards
...
fn set_document_title(title: &str) {
    if let Some(window) = web_sys::window() {
        ...
        if changed {
            ...
            // Post to parent window (FreeTalk runs inside an iframe)
...
/// Get the current board name (decrypted if private)
fn get_current_board_name() -> Option<String> {
    let current_board = CURRENT_BOARD.read();
    let owner_key = current_board.owner_key?;

    let boards = BOARDS.read();
    let board_data = boards.map.get(&owner_key)?;
    ...
    let sealed_name = &board_data
        ...
        .display
        .name;
    match unseal_bytes_with_secrets(sealed_name, &board_data.secrets) {
        Ok(bytes) => Some(String::from_utf8_lossy(&bytes).to_string()),
        Err(_) => Some(sealed_name.to_string_lossy()),
    }
}

/// Count unread messages in a board.
/// Only counts display messages (non-action, non-deleted) from other users.
pub fn count_unread_messages_in_board(board_owner: &ed25519_dalek::VerifyingKey) -> usize {
    let boards = BOARDS.read();
    let Some(board_data) = boards.map.get(board_owner) else {
        return 0;
    };

    let self_member_id: MemberId = board_data.self_sk.verifying_key().into();
    let last_read_id = board_data.last_read_message_id.as_ref();
    ...
    for msg in board_data.board_state.recent_messages.display_messages() {
        let msg_id = msg.id();
...
pub fn update_document_title() {
    ...
    *TOTAL_UNREAD_COUNT.write() = unread_count;
    ...
    let title = match (board_name, is_visible, unread_count) {
        ...
        // No board selected, tab visible (or no unread) - show app name
        (None, true, _) => APP_NAME.to_string(),
        (None, false, 0) => APP_NAME.to_string(),

        // No board selected, tab hidden with unread messages - show count
...
/// Mark all messages in the current board as read
pub fn mark_current_board_as_read() {
    let current_board = CURRENT_BOARD.read();
    let Some(owner_key) = current_board.owner_key else {
        return;
    };
    ...
    let latest_message_id = {
        let boards = BOARDS.read();
        let Some(board_data) = boards.map.get(&owner_key) else {
            return;
        };
    ...
    // Check if we need to update
    {
        ...
        if let Some(board_data) = boards.map.get(&owner_key) {
            if board_data.last_read_message_id.as_ref() == Some(&new_last_read_id) {
    ...
    BOARDS.with_mut(|boards| {
        if let Some(board_data) = boards.map.get_mut(&owner_key) {
            info!("Marking board as read up to message {:?}", new_last_read_id);
            board_data.last_read_message_id = Some(new_last_read_id);
        }
    });
    ...
    wasm_bindgen_futures::spawn_local(async {
        if let Err(e) = save_boards_to_delegate().await {
            warn!("Failed to save boards after marking as read: {}", e);
        }
    });
...
fn on_visibility_change() {
    ...
    *DOCUMENT_VISIBLE.write() = is_visible;
    ...
    if is_visible {
        // Tab became visible - mark current board as read
...
/// Call this in the App component to ensure title updates when board changes.
#[component]
pub fn DocumentTitleUpdater() -> Element {
    // Track current board changes
    let current_board = CURRENT_BOARD.read();
    let _current_board_key = current_board.owner_key;

    // Track board data changes (for message count updates)
    let boards = BOARDS.read();
    let _boards_version = boards.map.len(); // Simple trigger for reactivity

    // Update title on changes
    use_effect(move || {
        update_document_title();

        // If visible and a board is selected, mark as read
        if *DOCUMENT_VISIBLE.read() && CURRENT_BOARD.read().owner_key.is_some() {
            mark_current_board_as_read();
        }
    });
